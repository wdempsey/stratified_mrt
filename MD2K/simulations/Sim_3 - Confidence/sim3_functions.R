...
daily.sim <- function(N, pi, tau, C, P.0, P.treat, T, window.length, min.p, max.p) {
  ## Simulate the Markov chain given length (T), transition matrix (P),
  ## and initial point (init)
  ## C is a vector for confidence
  delta.1 = rbeta(1,shape1 = P.0[1,1]*C, shape2 = (1-P.0[1,1])*C)-P.0[1,1]
  delta.2 = rbeta(1,shape1 = P.0[2,2]*C, shape2 = (1-P.0[2,2])*C)-P.0[2,2]
  Delta = matrix(c(delta.1, -delta.1,-delta.2, delta.2), nrow = 2, byrow = TRUE)
  P.conf = P.0 + Delta
  ...
  while (t <= T+window.length) {
    if(A.t[t-1] ==0) {
      X.t[t] = sample(1:nrow(P.conf), size = 1, prob = P.conf[X.t[t-1],])
      if(t > T) {
        I.t[t] = 0; rho.t[t] = 0
      } else{
...
daily.data <- function(N, pi, tau, C, P.0, daily.treat, T, window.length, min.p, max.p, treatment.data){
  ...
  inside.fn <- function(day) {
    ...
    H.t = daily.sim(N, pi, tau, C, P.0, P.treat, T, window.length, min.p, max.p)
...
full.trial.sim <- function(N, pi, tau, C, P.0, daily.treat, T, window.length, min.p, max.p,treatment.data) {
...
MRT.sim <- function(num.people, N, pi, tau, C, P.0, daily.treat, T, window.length, min.p, max.p, treatment.data) {
