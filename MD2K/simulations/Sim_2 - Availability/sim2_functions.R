...
daily.data <- function(N, pi, tau.daily, P.0, daily.treat, T, window.length, min.p, max.p, treatment.data){
  # Generate the daily data for a participant given all the inputs!

    inside.fn <- function(day) {
        tau = rep(tau.daily[day],length(N))
...
full.trial.sim <- function(N, pi, tau.daily, P.0, daily.treat, T, window.length, min.p, max.p,treatment.data) {
  # Generate the full trial simulation using a vector of the daily treatment effects
  foreach(i=1:length(daily.treat), .combine = "rbind", .packages = c("foreach", "TTR","expm","zoo")) %dopar% daily.data(N, pi, tau.daily, P.0, daily.treat, T, window.length, min.p, max.p,treatment.data)(i)
}

MRT.sim <- function(num.people, N, pi, tau.daily, P.0, daily.treat, T, window.length, min.p, max.p) {
  # Do the trial across people!!
  treatment.data = potential.effects(P)
  output = foreach(i=1:num.people, .combine = "rbind", .packages = c("foreach", "TTR","expm","zoo")) %dopar% cbind(i,full.trial.sim(N, pi, tau.daily, P.0, daily.treat, T, window.length, min.p, max.p,treatment.data))
