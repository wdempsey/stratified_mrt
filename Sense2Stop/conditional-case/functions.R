...
full.trial.sim <- function(N, pi, P.0, P.treat.list, T, window.length, min.p, max.p) {
  ...
  foreach(i=1:length(P.treat.list), .combine = "rbind") %do% daily.data(N, pi, P.0, P.treat.list, T, window.length, min.p, max.p)(i)
}
...
barsigma.estimation <- function(num.iters, N, pi, P.0, P.treat.list, T,
  ...
  output = foreach(i=1:num.iters,.combine = "+") %do% full.trial.barsigma.sim(N, pi, P.0, P.treat.list, T, window.length, min.p, max.p)
  return(output)
}
...
tildepr.estimation <- function(num.iters, N, pi, P.0, P.treat.list,
  ...
  output = foreach(i=1:num.iters,.combine = "+") %do% full.trial.tildepr.sim(N, pi, P.0, P.treat.list, T, window.length, min.p, max.p)
  return(output)
}
